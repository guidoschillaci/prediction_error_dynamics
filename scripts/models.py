...
class Models:
    ...
    def build_forward_code_model(self, param):
        ...
        x = Dense(param.get('code_size') * 10, activation=self.activation_positive_tanh)(x)
        code = Dense(param.get('code_size'), activation = self.activation_positive_tanh, name='output')(x)
    ...
    def build_inverse_code_model(self, param):
        ...
        x = Dense(param.get('code_size'), activation=self.activation_positive_tanh)(input_code)
        x = Dense(param.get('code_size') * 10, activation=self.activation_positive_tanh)(x)
        #x = Dropout(0.2)(x)
        x = Dense(param.get('code_size') * 10, activation=self.activation_positive_tanh)(x)
        #x = Dropout(0.2)(x)
