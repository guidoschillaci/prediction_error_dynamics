...
class IntrinsicMotivation():
	...
	def update_mse_dynamics(self, mse):
		...
		if len(self.buffer_mse) < 2:  # not enough prediction error to calculate the regression
			...
			if not self.param.get('fixed_expl_noise'):
		...
		if self.param.get('fixed_expl_noise'):
			self.std_dev_exploration_noise.append(self.param.get('im_std_exploration_noise_if_fixed'))
	...
	def get_std_dev_exploration_noise(self):
		...
		if self.param.get('fixed_expl_noise'):
			return self.param.get('im_std_exploration_noise_if_fixed')
		else:
			if len(self.std_dev_exploration_noise) == 0:
